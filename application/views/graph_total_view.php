<script>           
			...
			$pst = percentage(($row['inlike']+$row['outlike']),($row['inhate']+$row['outhate']));
			...
			?>{
				...
				<?php  $Jm=date("m", $timestamp)-1;// months in javascript statr from 0 to 11 so I should make -1 to get right data?>
                date: new Date(<?php echo date("Y", $timestamp).','.$Jm.','.date("d", $timestamp);?>),
                like_pst:<?php echo $pst['likepst'];?>,			
				hate_pst:<?php echo $pst['hatepst'];?>,
				like: <?php echo ($row['inlike']+$row['outlike']); ?>,
				hate: <?php echo ($row['inhate']+$row['outhate']); ?>
            },<?php endforeach; ?>];

            AmCharts.ready(function () {
                var chart = new AmCharts.AmSerialChart();
                chart.dataProvider = lineChartData_total; 
                ...
                // WRITE
                chart.write("chartdiv-total");
            });
</script>
        <div id="chartdiv-total" class="graph"></div>
